...
* Search for `nightly` and replace it with `26.0.2`, if the release is performed against the `Keycloak 26.0.2` build.
* Search for `keycloak-999.0.0-SNAPSHOT` and replace it with `keycloak-26.0.2`, if the release is performed against the `Keycloak 26.0.2` build.
+
